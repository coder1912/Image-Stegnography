...
Status do_decoding(DecodeInfo *decInfo)
{
    ...
    if (open_decode_files(decInfo) == e_success)
    {
        ...
        {
            printf("INFO: Decoded magic string is success\n");
                    
            //To check whether decoded file extension size is decoded successfully or not
            if (decode_secret_file_extn_size(decInfo) == e_success)
            {
                printf("INFO: Decoded secret file extension size is success\n");
                ...
                {
                    printf("INFO: Decoded secret file extension is success\n");
                
                    //To check whether decoded file size is decoded successfully or not
                    if (decode_secret_file_size(decInfo) == e_success)
                    {
                        printf("INFO: Decoded file size is success\n");

                        //To check whether decoded data to file  successful or not
                        if (decode_secret_file_data(decInfo) == e_success)
                        {
                            printf("INFO: Decoded the secret data is success\n");
                            return e_success;
                        }
                        else
                        {
                            printf("INFO: Decoded the secret data is failure\n");
                            return e_failure;
                        }
                    }
                    else
                    {
                        printf("INFO: Decoded file size is failure\n");
                        return e_failure;
                    }
                }
                else
                {
                    printf("INFO: Decoded secret file extension is failure\n");
                    return e_failure;
                }
            }
            else
            {
                printf("INFO: Decoded secret file extension size is failure\n");
                return e_failure;
            }
        }
        else
        {
            printf("INFO: Decoded magic string is failure\n");
            return e_failure;
        }
    }
    else
    {
        printf("INFO: failure in Opening the files\n");
        return e_failure;
    }
    return e_success;
}
...
Status decode_magic_string(const char *magic_string, DecodeInfo *decInfo)
{
    ...
    if (decode_byte_from_lsb(decInfo->decode_data, decInfo->image_data) == e_success)
    {
        ...
        if (decInfo->decode_data[0] == '$')
        {
            ...
            printf("INFO: Decoded data of magic string matched\n");
            return e_success;
        }
        else
        {
            fprintf(stderr,"INFO: Error --> Decoded data of magic string not matched\n");
            return e_failure;
        }
    }
    else
...
Status decode_secret_file_extn(uint extn_size, DecodeInfo *decInfo)
{
    ...
    printf("INFO: Decoded secret file extension matched\n");
    return e_success;
}
