...
Status do_encoding(EncodeInfo *encInfo)
{
    ...
    if (open_files(encInfo) == e_success)
    {
        ...
        if (check_capacity(encInfo) == e_success)
        {
            ...
            {
                printf("INFO: Copied bmp header is success\n");
                ...
                if (encode_magic_string(MAGIC_STRING, encInfo) == e_success)
                {
                    ...
                    if (encode_secret_file_extn_size(strlen(encInfo -> extn_secret_file), encInfo -> fptr_src_image, encInfo -> fptr_stego_image) == e_success)
                    {
                        ...
                        {
                            printf("INFO: Encoded secret file extension was not success\n");
                            return e_failure;
                        }
                    }
                    else
            ...
            {
                printf("INFO: Copied bmp header was failure\n");
            }
        }
        else
        {
            printf("INFO: Check capacity was failure\n");
            return e_failure;
        }
    }
    else
    {
        printf("INFO: Open Files was failure\n");
        return e_failure;
    }    
    return e_success;
}
